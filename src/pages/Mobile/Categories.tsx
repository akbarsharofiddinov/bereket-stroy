...
import { setAllCategories, setSelectedCategory } from '@/store/categorySlice';
...
const Categories: React.FC = () => {
  ...
  const { searchModal } = useAppSelector(state => state.projectSlice);
  const { favorites, cart } = useAppSelector(state => state.productSlice)
  ...
  return (
    <>
      <div className="mobile-categories_page">
          ...
          <div className="all-categories">
            {allCategories.length ? (
              allCategories.map((item, index) => (
                <Link to={`${item.slug}`} key={index} onClick={() => dispatch(setSelectedCategory(item))}>
                  <span>
                  ...
                  <span>
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M9.00005 6C9.00005 6 15 10.4189 15 12C15 13.5812 9 18 9 18" stroke="black" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  </span>
                </Link>
              ))
        ...
        <div className="mobile-footer_navigation_bar">
          <div className="inner">
            <NavLink to={"/"} onClick={() => dispatch(setCatalogModal(false))}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
